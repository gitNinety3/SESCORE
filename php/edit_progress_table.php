...
if (isset($_GET['id'])) {
  ...
  $result = mysqli_query($conn, $sql);
	...
	if (empty($workid)) {
		header("Location: ../edit_jboard.php?id=$id&error=Error: 'workid' is missing");
	} else if (empty($partid)) {
		header("Location: ../edit_jboard.php?id=$id&error=Error: 'partid' is missing");
	} else if (empty($quantity)) {
		header("Location: ../edit_jboard.php?id=$id&error=Error: 'Quantity' is missing");
	} else {

       $sql = "UPDATE workorders SET workid = '$status' WHERE id = $id ";
       $result = mysqli_query($conn, $sql);
       if ($result) {
